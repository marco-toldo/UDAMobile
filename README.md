...
> Toldo M., Michieli U., Agresti G., Zanuttigh P., *'Unsupervised Domain Adaptation for Mobile Semantic Segmentation based on Cycle Consistency and Feature Alignment'*

---

## Setup

1. Download the pre-trained model on the Pascal VOC 2012 of the MobileNetV2 network from:
...
**1. Preliminary training stage**

A preliminary training stage of the segmentation network is required. The training on the GTA dataset can be run with the following command:
```bash
...
Then the domain adaptation framework can be trained in the GTA-Cityscapes scenario by running:
```bash
python deeplab/train.py \
